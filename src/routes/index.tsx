    ...
    <PhoneFrame>
      <div className="bg-darkgrad relative flex flex-1 flex-col justify-between overflow-hidden min-h-screen">
        ...
        <div className="relative flex flex-1 flex-col items-center justify-center px-7 text-center pt-8">
          ...
          {/* 3 simple steps — pehli baar aane wale user ke liye */}
          <ol className="mt-7 w-full space-y-2 text-left">
            {[
              "Account banayein",
              "Email par aaya 6-digit code daalein",
              "Admin approve karte hi app khul jayega",
            ].map((step, i) => (
              <li
                key={i}
                className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-3.5 py-2.5 text-sm text-warm"
              >
                <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-goldgrad text-xs font-bold text-[#3A2508]">
                  {i + 1}
                </span>
                {step}
              </li>
